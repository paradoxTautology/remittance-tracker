...
export default function Dashboard({ claims, payers, onNavigate, onViewClaims, workLogEntries = [], todos = [], onAddTodo, onToggleTodo, onRemoveTodo, onClearDoneTodos, view }) {
    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
        {[
          ...
          <div key={c.l} className="card" style={{ padding: 18, borderLeft: `3px solid ${c.c}`, cursor: "pointer" }} onClick={() => onNavigate("remittance")}>
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px", color: "var(--text-muted)", marginBottom: 6 }}>{c.l}</div>
            <div className="mono" style={{ fontSize: 22, fontWeight: 700 }}>{c.v}</div>
            <div style={{ fontSize: 11, color: "var(--text-secondary)", marginTop: 3 }}>{c.s}</div>
          </div>
      ...
      <div className="card" style={{ padding: 18, marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px" }}>To-Do</span>
            {openTodos > 0 && (
        ...
        <TodoPanel todos={todos} onAdd={onAddTodo} onToggle={onToggleTodo} onRemove={onRemoveTodo} onClearDone={onClearDoneTodos} />
      </div>

      {/* Quick link to Remittance */}
      <div
        className="card"
      ...
      >
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 4 }}>Remittance Tracker</div>
          <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>
            {stats.total} claims · {payers.length} payers · {stats.deniedCount} denied
          </div>
        </div>
