...
export default function Dashboard({ claims, payers, onNavigate, onViewClaims, workLogEntries = [] }) {
  ...
  // Reason code frequency — ALL codes
  const codeCounts = {};
  claims.forEach((c) =>
  ...
  return (
    <>
      ...
      {/* Top reason codes — with Show All toggle */}
      {allCodes.length > 0 && (
        <div className="card" style={{ padding: 18, marginBottom: 16 }}>
      ...
      {/* Per-payer table — with color-coded columns */}
      {payers.length > 0 && (
              ...
              <tr>
                {["Payer", "Claims", "Billed", "Paid", "Denied"].map((h, i) => (
                  <th key={h} style={{
                    ...
                    fontSize: 10,
                    fontWeight: 700,
                    color: i === 1 ? "#d4a843" : i === 2 ? "#6ca6d6" : "var(--text-muted)",
              ...
              {payers.map((p, rowIdx) => {
                ...
                return (
                  <tr key={p} style={{ background: rowIdx % 2 === 0 ? "transparent" : "rgba(255,255,255,0.02)" }}>
                    ...
                    <td className="mono" style={{ textAlign: "right", fontSize: 12, color: "#6ca6d6" }}>
                      {formatDollar(pc.reduce((s, c) => s + parseDollar(c.billed), 0))}
                    </td>
                    <td className="mono" style={{ textAlign: "right", fontSize: 12, color: "var(--green)" }}>
                      {formatDollar(pc.reduce((s, c) => s + Math.max(0, parseDollar(c.prov_paid)), 0))}
                    </td>
                    <td className="mono" style={{ textAlign: "right", fontSize: 12, color: "var(--red)" }}>
